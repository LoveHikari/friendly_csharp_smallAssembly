...
namespace Common
{
    ...
    public class IniHelper : Dictionary<object, Dictionary<object, object>>
    {
        ...
        public void Load(string iniPath)
        {
            ...
            if (bufLen > 0)
            {
                string sb = Encoding.GetEncoding("gbk").GetString(buffer);
                string[] lines = sb.Split("\0".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
            ...
            foreach (var key in keysList)
            {
                ...
                if (bufLen > 0)
                {
                    ...
                    foreach (string line in lines)
                    {
                        int index = line.IndexOf("=", StringComparison.CurrentCulture);
                        value.Add(line.Substring(0, index), line.Substring(index + 1, line.Length - index - 1));
                    }
                    this.Add(key, value);
                }
            }
        ...
        public void SaveAs(string iniPath)
        {
            ...
            System.IO.StreamWriter sw = new System.IO.StreamWriter(iniPath, false,Encoding.GetEncoding("gbk"));
            sw.Write("#表格配置档案");
            sw.Close();
            sw.Dispose();
